...
class MLP:
    def __init__(self, data, depth, nhidden, ntarget, nbatch, maxepoch, learning_rate):
        ...
        self.nX = len(self.data.columns) - self.nY #banyaknya node/feature icput
        ...
        for i in range(self.depth + 1):
            if (i == 0): #dari icput layer ke hidden layer pertama
                self.weights.append(cp.random.rand(self.nX + 1, self.nhidden)*math.sqrt(2/(self.nX + self.nhidden)))
            elif (i == depth): #dari hidden layer terakhir ke output
                self.weights.append(cp.random.rand(self.nhidden + 1, self.nY)*math.sqrt(2/(self.nhidden + self.nhidden)))
            else: #dari hidden layer ke hidden layer
                self.weights.append(cp.random.rand(self.nhidden + 1, self.nhidden)*math.sqrt(2/(self.nhidden + self.nY)))                                    
        #Proses training
        self.train()
        ...
        for column in columns:
            ...
            if (not self.isContinue(self.data[column])):
                ...
                while i <= 1:
                    ...
                    i += space 
                local_class_array = cp.array(local_class_array)
    ...
    def feedforward(self, X):
        self.hidden = []
        icp = cp.array(X.to_numpy())
        for i in range(self.depth + 1):
            icp = self.sigmoid(cp.dot(self.concat(icp), self.weights[i]))
            if (i < self.depth): 
                self.hidden.append(icp)
            else:
                out = icp
        return out
    ...
    def concat(self, matriks):
        ...
        for key, value in enumerate(matriks):
            temp.append(cp.concatenate([cp.array([1]), value]))
    ...
    def train(self):
        for e in range(self.maxepoch):
            for i in range(0, len(self.data), self.nbatch):
                ...
                target = cp.array(self.data.iloc[i:i+self.nbatch, len(self.data.columns) - self.nY:].to_numpy())
                global_output = output.copy()
                global_icput = cp.array(self.data.iloc[i:i+self.nbatch, :len(self.data.columns) - self.nY].copy().to_numpy())
                delta = cp.multiply(cp.multiply(output, 1 - output), target - output)
                deltaw = self.learning_rate*cp.dot(self.concat(self.hidden[self.depth - 1]).transpose(), delta)
                self.weights[self.depth] = self.weights[self.depth] + deltaw
                for j in range(self.depth - 1, -1, -1):
                    output = self.hidden[j]
                    sigma = cp.dot(delta, self.weights[j + 1][1:,:].transpose())
                    delta = cp.multiply(cp.multiply(output, 1 - output), sigma)
                    if (j == 0):
                        deltaw = self.learning_rate*cp.dot(self.concat(global_icput).transpose(), delta)
                    else:  
                        deltaw = self.learning_rate*cp.dot(self.concat(self.hidden[j - 1]).transpose(), delta)
                    self.weights[j] = self.weights[j] + deltaw
            # print(e, 0.5*cp.mean(cp.square(target - output)))
            print(e, 0.5*cp.mean(cp.square(target - output)), self.test(self.testdata))
    ...
    def predict(self, X):
        ...
        for i in range(len(X)):
            ...
            for key, column in enumerate(self.target):         
                idx = cp.argmax(1 - cp.absolute(self.class_array[column] - output[i, key]))
                row_result.append(self.outmapping[column][int(idx)].split('::')[1])
...
if __name__ == "__main__":
    ...
    mask = cp.random.rand(len(iris)) < 0.8
